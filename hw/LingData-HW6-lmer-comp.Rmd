...
```{r 1.1, echo = FALSE}
ggplot(upsid, aes(x=consonants, y=vowels)) + geom_point() + theme_bw()
```
...
```{r 1.5, echo=FALSE}
ggplot(upsid, aes(x=consonants, y=vowels)) + geom_point() + theme_bw() + geom_line(data = augment(fit1), aes(x = consonants, y = .fitted), color = "blue")
```
...
```{r 2.1, echo=FALSE}
ggplot(upsid, aes(x=consonants, y=vowels, color=area)) + geom_point() + theme_bw() + stat_ellipse()
```
...
ggplot(upsid, aes(x=consonants, y=vowels, color=area)) + geom_point() + theme_bw() + geom_line(aes(consonants, model2, color = area))
...
```{2.4}
Consonants variable is still significant since it has p-value = 0.000905 which is less than the significance level of 5%. After we added area random effect the p-value increased. All the lines on the graph show that the more consonants we have, the more vowels there are in a language. The lines are all aligned and show a positive correlation that the number of vowels depends on the number of consonants in languages on all the continents.
```
...
ggplot(upsid, aes(x=consonants, y=vowels)) + geom_point() + theme_bw() + geom_line(aes(consonants, model3, color = area))
```
...
```{3.3}
Consonants variable is not significant since its p-value is 0.48 which is more than the significance level of 5%. After the addition of random slopes our graph changed a lot. Now only several lines are similar and have a positive correlation. The languages of North America and Papua seem to have a neutral correlation. The languages of Australia have a negative one. 
```
...
```{r 4.1}
fit4 <- lmer(vowels ~ consonants + (1+consonants|area), data = upsid)
```
...
ggplot(upsid, aes(x=consonants, y=vowels)) + geom_point() + theme_bw() + geom_line(aes(consonants, model4, color = area))
```
...
```{r 4.3}
summary(fit4)
Conconants variable is not significant as it has p-value=0.46182. 
The addition of random effects causes the difference between the prediction lines in these three models.
```
